...
for i=1:length(a)-1
    ...
    hsTNT(end+1:end+length(a(i)+3:a(i+1)+1))=Troponinexcel.VarName10(a(i)+3:a(i+1)+1); %predictors are CBCs at max(HsTNT)
...
for i = 1:length(uniquePat)
  ...
  for j=1:lengthPat(i)-1
  dataCBC(end+1,1:end-1) = data(firstidx(i)+j,1:end-1);%-data(firstidx(i)+j-1,1:end-1);
  dataCBC(end,end) = data(firstidx(i)+j,end)-data(firstidx(i)+j-1,end);
  end
end
dataCBC(1,:)=[];
for i = 1:numel(dataCBC(1,:))-1
dataCBC(:,i) = (dataCBC(:,i) - mean(dataCBC(:,i)))./std(dataCBC(:,i));
end
...
p(i) = stats(3);
 ynorm = (yy2-min(yy2))./(max(yy2)-min(yy2));
    xnorm = (dataCBC(:,i)-min(dataCBC(:,i)))./(max(dataCBC(:,i))-min(dataCBC(:,i)));
...
for i = 1:length(uniquePatRBC)
    ...
    for j=1:lengthPat(i)-1
        dataRBC_(end+1,1:end-1) = dataRBC(firstidx(i)+j,1:end-1);%-dataRBC(firstidx(i)+j-1,1:end-1);
        dataRBC_(end,end) = dataRBC(firstidx(i)+j,end)-dataRBC(firstidx(i)+j-1,end);
    end
end
dataRBC_(1,:)=[];
for i = 1:numel(dataRBC_(1,:))-1
dataRBC_(:,i) = (dataRBC_(:,i) - mean(dataRBC_(:,i)))./std(dataRBC_(:,i));
end
for i = 1:numel(dataRBC_(1,:))-1
...
pRBC(i) = stats(3);
 ynorm = (yy2-min(yy2))./(max(yy2)-min(yy2));
    xnorm = (dataRBC_(:,i)-min(dataRBC_(:,i)))./(max(dataRBC_(:,i))-min(dataRBC_(:,i)));
...
for i = 1:length(uniquePatplt)
    ...
    for j=1:lengthPat(i)-1
        dataplt_(end+1,1:end-1) = dataplt(firstidx(i)+j,1:end-1);%-dataplt(firstidx(i)+j-1,1:end-1);
        dataplt_(end,end) = dataplt(firstidx(i)+j,end)-dataplt(firstidx(i)+j-1,end);
    end
end
dataplt_(1,:)=[];
for i = 1:numel(dataplt_(1,:))-1
dataplt_(:,i) = (dataplt_(:,i) - mean(dataplt_(:,i)))./std(dataplt_(:,i));
end
...
pPLT(i) = stats(3);
 ynorm = (yy2-min(yy2))./(max(yy2)-min(yy2));
    xnorm = (dataplt_(:,i)-min(dataplt_(:,i)))./(max(dataplt_(:,i))-min(dataplt_(:,i)));

        [rPLT,cp] = corrcoef(xnorm,ynorm);
    corrpPLT(:,:,i)=cp;
    corrPLT(:,:,i) = rPLT;
end
Label = [cellstr('TNT vs MPV'), cellstr('TNT vs \mu_1'),cellstr('TNT vs \mu_2'),cellstr('TNT vs \sigma_1'),cellstr('TNT vs \sigma_2'),cellstr('TNT vs Kurtosis_1'),...
    cellstr('TNT vs Kurtosis_2'),cellstr('TNT vs Skewness_1'),cellstr('TNT vs Skewness_2'),cellstr('TNT vs Mode_1'),cellstr('TNT vs Mode_2'),...
    cellstr('TNT vs Median_1'),cellstr('TNT vs Median_2'),cellstr('TNT vs Interquart_1'),cellstr('TNT vs Interquart_2'),cellstr('TNT vs corr_{12}')];
